...
const WalletView: React.FC<WalletViewProps> = ({ portfolio, onConnectWallet, onWalletSend }) => {
    ...
    const handleCreateWallet = async () => {
        ...
        try {
            const data = await apiService.createWallet();
            setSecretPhrase(data.secretPhrase);
            setWalletStatus('generated');
        } catch (error) {
            console.error("Failed to create wallet:", error);
            setWalletStatus('none'); // Reset on error
            // a toast notification would be good here
        }
    };
        ...
        return (
             <div className="p-6 lg:p-8 flex items-center justify-center h-full">
                <Card className="max-w-lg w-full text-center p-8">
                 ...
                 {walletStatus === 'generated' && <SecretPhraseModal secretPhrase={secretPhrase} status={walletStatus} onConfirmBackup={() => setWalletStatus('backed_up')} onClose={() => setWalletStatus('none')} />}
             </div>
        )
    }
